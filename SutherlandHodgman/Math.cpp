...
bool sfmath::RayLineIntersect(const Ray& ray, const sf::Vector2f a, const sf::Vector2f b, sf::Vector2f& intersectPoint)
{
	...
	{
		t = roundf(t * 100) / 100;
		intersectPoint = ray.origin + (float)t * ray.direction;
		return true;
	}
...
bool  sfmath::RayRayIntersect(const Ray& rayA, const Ray& rayB, sf::Vector2f& intersectPoint)
{
	//Cross product of the directions
	float dirCross = Cross(rayA.direction, rayB.direction);
	if (dirCross == 0)
		return false;

	sf::Vector2f originDiff = rayB.origin - rayA.origin;
	float t = (Cross(originDiff, rayB.direction)) / dirCross;
	float u = (Cross(originDiff, rayA.direction)) / dirCross;
...
bool sfmath::InfLineInfLineIntersect(const Ray& rayA, const Ray& rayB, sf::Vector2f& intersectPoint)
{
	//Cross product of the directions
	float dirCross = Cross(rayA.direction, rayB.direction);
	if (dirCross == 0)
